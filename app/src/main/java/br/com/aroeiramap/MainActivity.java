...
public final class MainActivity extends Activity {
    ...
    private void requestPermissions(){
        if(checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)==PackageManager.PERMISSION_GRANTED || checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION)==PackageManager.PERMISSION_GRANTED){status.setText("Ready — tap START");return;}
    ...
    public void onRequestPermissionsResult(int r,String[] p,int[] g){super.onRequestPermissionsResult(r,p,g);if(r==REQ&&(checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)==PackageManager.PERMISSION_GRANTED||checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION)==PackageManager.PERMISSION_GRANTED))status.setText("Ready — tap START");else if(r==REQ)status.setText("Location permission denied");}
    private void toggle(){if(checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED&&checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION)!=PackageManager.PERMISSION_GRANTED){requestPermissions();return;}tracking=!tracking;if(tracking){startForegroundService(new Intent(this,TrackingService.class));status.setText("Tracking — acquiring GPS/network fix…");}else{startService(new Intent(this,TrackingService.class).setAction(TrackingService.ACTION_STOP));status.setText("Tracking stopped");}}
