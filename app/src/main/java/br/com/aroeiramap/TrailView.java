...
public final class TrailView extends View {
    ...
    public TrailView(Context c, AttributeSet a){super(c,a);p.setStrokeCap(Paint.Cap.ROUND);setBackgroundColor(Color.rgb(239,242,232));}
    ...
    protected void onDraw(Canvas c){super.onDraw(c);if(!hasCenter){p.setColor(Color.DKGRAY);p.setTextSize(38);p.setTextAlign(Paint.Align.CENTER);c.drawText("Waiting for GPS…",getWidth()/2f,getHeight()/2f,p);return;}
        ...
        p.setColor(Color.rgb(25,92,181));p.setStrokeWidth(4);c.drawPath(path,p);
        ...
        p.setColor(Color.rgb(30,30,30));c.drawCircle(getWidth()/2f,getHeight()/2f,7,p);
    }
    private void grid(Canvas c){p.setStrokeWidth(1);p.setColor(Color.argb(35,0,0,0));float step=50/metersPerPx;for(float x=getWidth()/2f%step;x<getWidth();x+=step)c.drawLine(x,0,x,getHeight(),p);for(float y=getHeight()/2f%step;y<getHeight();y+=step)c.drawLine(0,y,getWidth(),y,p);p.setColor(Color.DKGRAY);p.setTextSize(24);p.setTextAlign(Paint.Align.LEFT);c.drawText("50 m grid • green = searched corridor",18,34,p);}
